...
2. 时间单位统一：全部转换为秒(数值)或天:小时:分钟:秒(字符串)
3. 消除模糊表述：将"快速"、"大量"等转换为具体数值
4. 输出的是一段简介规范的自然语言描述
...
示例输出：为我生成一个有向图，节点包括源（源），缓冲区，加工工位（工位），传送器（传送器），测试工位（工位），合格库存（物料终结）和废品库存（物料终结），源，加工工位，缓冲区，传送器，测试工位依次为串联结构，测试工位分别连接合格库存与废品库存，源的时间间隔为10分钟，起始时间为0，结束时间为1天，缓冲区容量为8，传送器长度为2米，宽度为0.5米，速度为1m/s，容量为2，加工工位处理时间为正态分布平均值200，标准差30，故障间隔为2000，持续时间为200，缓冲区容量为8，测试工位的处理时间为1分钟，故障间隔时间为负指数分布，均值为2000，持续时间为负指数分布，均值为200，测试结果为合格率是70%，合格的产品输入合格库存，不合格的产品输入废品库存。
"""
...
if __name__ == "__main__":
    test_input = "本案例以缸盖装配支线为研究对象。缸盖装配支线的工艺流程如下：缸盖上线、拆卸凸轮轴盖、装气门油封、装缸盖气门、装止回阀、装气门弹簧、拧紧火花塞、装缸盖气门锁夹、激光锁夹检测、气门拍打、燃烧室密封性检测、缸盖下线，每两个装配工位之间是辊道"
